...
def test_statechart_returns_json(session, test_input="/statechart") -> None:
...
def test_statechart_checks_if_state_exists_in_json(session, test_input="/statechart") -> None:
    _session, settings = session
    replies = _session.get(settings.base_key_expr+test_input, zenoh.ListCollector(), target=zenoh.QueryTarget.ALL())
    for reply in replies():
        json.loads(reply.ok.payload.decode('utf-8'))["states"]

def test_statechart_checks_if_transition_exists_in_json(session, test_input="/statechart") -> None:
    _session, settings = session
    replies = _session.get(settings.base_key_expr+test_input, zenoh.ListCollector(), target=zenoh.QueryTarget.ALL())
    for reply in replies():
...
def test_trigger_accepts_timestamped_events(session, test_input) -> None:
    _session, settings = session
    replies = _session.get(settings.base_key_expr+test_input, zenoh.ListCollector(), target=zenoh.QueryTarget.ALL())
    for reply in replies():
...
def test_trigger_rejects_notallowed_events(session, test_input) -> None:
    _session, settings = session
    replies = _session.get(settings.base_key_expr+test_input, zenoh.ListCollector(), target=zenoh.QueryTarget.ALL())
    for reply in replies():
        json.loads(reply.ok.payload.decode('utf-8')) == "rejected"

def test_trigger_rejects_invalid_timestamp(session, test_input="/trigger?event=done") -> None:
    _session, settings = session
    replies = _session.get(settings.base_key_expr+test_input, zenoh.ListCollector(), target=zenoh.QueryTarget.ALL())
    for reply in replies():
...
def test_trigger_rejects_invalid_event(session, test_input) -> None:
    _session, settings = session
    replies = _session.get(settings.base_key_expr+test_input, zenoh.ListCollector(), target=zenoh.QueryTarget.ALL())
    for reply in replies():
        json.loads(reply.ok.payload.decode('utf-8'))["response_code"] == "rejected"
